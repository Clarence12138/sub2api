export default {
    ...
    subscriptions: {
      ...
      resetQuotaChooseWindows: "选择要为 '{user}' 重置的用量窗口。用量将归零并重新开始计算。",
      selectAtLeastOneWindow: '请至少选择一个要重置的用量窗口',
      quotaResetSuccess: '配额重置成功',
      failedToResetQuota: '重置配额失败',
      bulk: {
        selectGroups: '按分组选中',
        selectionSummary: '已选 {groups} 个分组、{subscriptions} 个订阅，排除 {excluded} 个',
        actualSelectionCount: '实际可重置 {count} 个',
        clearSelection: '清空选择',
        resetSelected: '批量重置',
        confirmHint: '确认目标范围，并选择要重置的用量窗口。分组会覆盖所有分页中的有效订阅。',
        previewLoading: '正在计算实际影响范围…',
        previewSummary: '匹配 {total} 个订阅，可重置 {valid} 个，失败 {failed} 个',
        previewFailed: '无法预览批量重置范围',
        failureTitle: '以下订阅重置失败，可修正后重试：',
        resetSuccess: '已成功重置 {count} 个订阅',
        partialFailure: '批量重置部分完成：成功 {success} 个，失败 {failed} 个'
      },
