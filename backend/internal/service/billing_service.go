...
func preferOpenAISubscriptionServiceTierBilling(model string) bool {
	...
	return strings.HasPrefix(lower, "gpt-") || strings.Contains(lower, "codex")
}
...
func usePriorityServiceTierPricing(serviceTier string, pricing *ModelPricing, model string) bool {
	if preferOpenAISubscriptionServiceTierBilling(model) {
		return false
	}
	if pricing == nil {
		return false
...
func serviceTierCostMultiplier(serviceTier, model string) float64 {
	switch normalizeBillingServiceTier(serviceTier) {
	case "priority", "fast":
		if usesOpenAIChatGPTFastCreditMultiplier(model) {
			return openAIChatGPTFastCreditMultiplier
		}
...
func configuredServiceTierMultiplier(serviceTier string, pricing *ModelPricing, models ...string) float64 {
	if pricing != nil {
		switch normalizeBillingServiceTier(serviceTier) {
		case "priority", "fast":
			if pricing.FastMultiplier != nil {
	...
	return serviceTierCostMultiplier(serviceTier, model)
}
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	breakdown := s.computeTokenBreakdownForModel(input.Model, pricing, input.Tokens, input.RateMultiplier, input.ServiceTier, applyLongCtx)
...
// computeTokenBreakdown keeps the upstream package-level test contract for generic
// tier behavior. Model-specific subscription credit rules use the internal variant.
...
func (s *BillingService) computeTokenBreakdownForModel(
	...
	if usePriorityServiceTierPricing(serviceTier, pricing, model) {
		...
		tierMultiplier = configuredServiceTierMultiplier(serviceTier, pricing, model)
	}
	...
	if longContextPricingEligible {
		baselineCost = s.computeTokenBreakdownForModel(model, pricing, tokens, rateMultiplier, serviceTier, false)
...
func (s *BillingService) calculateCostInternalWithPolicy(
	...
	return s.computeTokenBreakdownForModel(model, pricing, tokens, rateMultiplier, serviceTier, longContextBillingEnabled), nil
}
