...
func TestOpenAIGatewayServiceRecordUsage_CanPreferResultRequestIDForWebSocketTurns(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	...
	ctx := context.WithValue(context.Background(), ctxkey.ClientRequestID, "ws-connection-client-id")
	err := svc.RecordUsage(ctx, &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_ws_turn_2",
			Usage: OpenAIUsage{
				InputTokens:  8,
				OutputTokens: 4,
			},
		...
		PreferResultRequestID: true,
	})

	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "resp_ws_turn_2", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "resp_ws_turn_2", usageRepo.lastLog.RequestID)
}
