...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		edgeName                  sql.NullString
		entryHost                 sql.NullString
	...
	if edgeName.Valid {
		log.EdgeName = &edgeName.String
	}
	if entryHost.Valid {
		log.EntryHost = &entryHost.String
	}
